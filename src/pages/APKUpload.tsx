...
const APKUpload = ({
  ...
  const [inputRules, setInputRules] = useState<Array<{
    id: string;
    label: string;
    rule: string;
  }>>([]);
  const [specialOperations, setSpecialOperations] = useState<Array<{
    id: string;
    element: string;
    operation: string;
  }>>([]);
  ...
  const mockPages = [{
    name: "启动页面",
    screenshot: mobileAppUI
  }, {
    name: "登录页面",
    screenshot: appScreen1
  }, {
    name: "主页面",
    screenshot: mobileAppUI
  }, {
  ...
  const BackgroundEffects = () => <div className="fixed inset-0 pointer-events-none overflow-hidden">
      ...
      {/* Mouse glow effect */}
      <div className="absolute w-96 h-96 bg-primary/5 rounded-full blur-3xl transition-all duration-300" style={{
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-8">
          <Card className="bg-background/80 backdrop-blur-xl border-primary/20 shadow-2xl">
            ...
            <CardContent className="space-y-8">
              {!uploadedFile && <div className="border-2 border-dashed border-primary/30 rounded-2xl p-16 text-center hover:border-primary/60 transition-all duration-300 cursor-pointer group bg-gradient-to-br from-primary/5 to-purple-500/5" onClick={() => fileInputRef.current?.click()}>
                  <div className="relative">
              ...
              {uploadedFile && <div className="space-y-6">
                  ...
                  <Button onClick={startAnalysis} className="w-full h-16 text-xl bg-gradient-to-r from-primary to-blue-500 hover:from-primary/90 hover:to-blue-500/90 transition-all duration-300 group">
        ...
        <div className="relative z-10 pt-12 px-8">
          ...
          <div className="max-w-7xl mx-auto">
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-12">
                  ...
                  <div className="relative group">
                    ...
                    <div className="w-96 h-[700px] bg-black rounded-3xl overflow-hidden shadow-2xl relative">
                      ...
                      <div className="h-full bg-white relative">
                        <img src={currentScreenshot} alt="App Screenshot" className="w-full h-full object-cover cursor-pointer hover:opacity-90 transition-opacity" onClick={() => {
              ...
              {showConfigCard ? <Card className="bg-background/80 backdrop-blur-xl border-primary/20 shadow-2xl flex flex-col h-[800px]">
                ...
                <CardContent className="flex-1 flex flex-col min-h-0">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        <div>
                          ...
                          <input type="text" value={pageLabel} onChange={e => setPageLabel(e.target.value)} className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="例如：登录页面" />
                        </div>
                        <div>
                          <label className="text-sm font-medium text-muted-foreground">页面描述</label>
                          <textarea value={pageDescription} onChange={e => setPageDescription(e.target.value)} className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-primary/50 resize-none" rows={3} placeholder="描述这个页面的主要功能和用途..." />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        {inputRules.map((rule, index) => <div key={rule.id} className="flex gap-2">
                        ...
                        }} className="flex-1 px-3 py-2 border border-border rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="输入框标签" />
                            <input type="text" value={rule.rule} onChange={e => {
                          const newRules = [...inputRules];
                          newRules[index].rule = e.target.value;
                          setInputRules(newRules);
                        }} className="flex-1 px-3 py-2 border border-border rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="输入规则" />
                            <Button variant="ghost" size="sm" onClick={() => {
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        {specialOperations.map((operation, index) => <div key={operation.id} className="flex gap-2">
                            <input type="text" value={operation.element} onChange={e => {
                          const newOps = [...specialOperations];
                          newOps[index].element = e.target.value;
                          setSpecialOperations(newOps);
                        }} className="flex-1 px-3 py-2 border border-border rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="页面元素" />
                            <select value={operation.operation} onChange={e => {
                          const newOps = [...specialOperations];
                    ...
                    <div className="border-t pt-4 flex-shrink-0">
                      ...
                      <div className="w-full bg-muted rounded-full h-1">
                        <div className="bg-gradient-to-r from-primary to-blue-500 h-1 rounded-full transition-all duration-300" style={{
                          width: `${discoveredPages.length / 10 * 100}%`
                        }}></div>
                      </div>
                      {discoveredPages.length > 0 && <div className="flex flex-wrap gap-1 mt-2">
                          {discoveredPages.map((page, index) => <Badge key={index} variant="secondary" className="text-xs px-1.5 py-0.5">
                              {page}
                            </Badge>)}
                        </div>}
                    </div>
                    ...
                    <div className="pt-4 border-t">
                      <Button onClick={() => handleScreenHelp(pageDescription)} className="w-full bg-gradient-to-r from-primary to-blue-500 hover:from-primary/90 hover:to-blue-500/90" disabled={!pageLabel.trim()}>
              ...
              </Card> : <Card className="bg-background/80 backdrop-blur-xl border-primary/20 shadow-2xl flex flex-col h-[800px]">
      ...
      <div className="relative z-10 pt-20 px-8">
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            return <div key={step.key} className={`p-6 rounded-xl border-2 transition-all duration-300 ${isActive ? 'bg-primary/10 border-primary/30' : isCompleted ? 'bg-success/10 border-success/30' : 'bg-muted/10 border-muted/30'}`}>
                  <div className="flex items-center gap-3 mb-3">
                    {isCompleted ? <CheckCircle className="h-6 w-6 text-success" /> : isActive ? <Clock className="h-6 w-6 text-primary animate-pulse" /> : <AlertCircle className="h-6 w-6 text-muted-foreground" />}
          ...
          {analysisStep === 'learning' && discoveredPages.length > 0 && <div className="space-y-4">
              <div className="flex items-center gap-3">
                <Smartphone className="h-6 w-6 text-primary" />
                <span className="text-lg font-medium">正在学习页面: {currentPage}</span>
              </div>
              <div className="flex flex-wrap gap-3">
                {discoveredPages.map((page, index) => <Badge key={index} variant="outline" className="bg-success/10 border-success/30 text-success text-sm px-3 py-1">
                    {page}
                  </Badge>)}
              </div>
            </div>}
          ...
          {analysisStep === 'complete' && <div className="space-y-8">
              ...
              {/* Flow Diagram */}
              <div className="border border-border rounded-xl bg-background/50 backdrop-blur-sm" style={{
            height: '600px'
          }}>
